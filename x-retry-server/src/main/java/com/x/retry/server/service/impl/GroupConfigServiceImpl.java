...
public class GroupConfigServiceImpl implements GroupConfigService {
    ...
    public Integer getConfigVersion(String groupName) {
        ...
        GroupConfig groupConfig = groupConfigMapper.selectOne(new LambdaQueryWrapper<GroupConfig>()
                .select(GroupConfig::getVersion)
                .eq(GroupConfig::getGroupName, groupName));
        if (Objects.isNull(groupConfig)) {
            throw new XRetryServerException("未查询到 [{}] 信息", groupName);
        }
    ...
    @Override
    @Transactional
    public ConfigDTO syncConfig(String groupId) {

        ConfigDTO configDTO = new ConfigDTO();
        configDTO.setGroupId(groupId);
        configDTO.setSceneBlacklist(configAccess.getBlacklist(groupId));

        List<NotifyConfig> notifyList = configAccess.getNotifyListConfigByGroupName(groupId);
        ...
        for (NotifyConfig notifyConfig : notifyList) {
            ...
            notify.setNotifyAddress(notifyConfig.getNotifyAddress());
            notifies.add(notify);
        }
