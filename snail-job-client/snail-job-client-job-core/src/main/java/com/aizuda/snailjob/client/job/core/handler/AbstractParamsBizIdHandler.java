...
/**
 * Base handler for BizId-based job operations.
...
public abstract class AbstractParamsBizIdHandler<H, R> extends AbstractJobRequestHandler<R> {
    ...
    @Getter
    private final JobBizIdApiRequest reqDTO;
    @Setter
    private H r;

    /**
     * Constructor with bizId and task type.
     *
     * @param bizId    the business identifier
     * @param taskType the task type enum
     */
    public AbstractParamsBizIdHandler(String bizId, JobTaskTypeEnum taskType) {
        this.reqDTO = new JobBizIdApiRequest();
        // 设置业务ID
