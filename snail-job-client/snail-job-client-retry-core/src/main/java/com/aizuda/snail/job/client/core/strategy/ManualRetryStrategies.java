                ...
                return Collections.singletonList(new RetryListener() {
                    ...
                    public <V> void onRetry(Attempt<V> attempt) {
                        if (attempt.hasResult()) {
                           EasyRetryLog.LOCAL.info("snail-job 手动创建重试数据成功，第[{}]次调度", attempt.getAttemptNumber());
                        }

                        if (attempt.hasException()) {
                            EasyRetryLog.LOCAL.error("snail-job 手动创建重试数据失败，第[{}]次调度 ", attempt.getAttemptNumber(),
